...
public enum CustomParticleEffect {
	TRAIL_1(){
		@Override public void display(Player p, long time){
			...
			if(time%15 == 0) p.getWorld().spawnParticle(Particle.BLOCK_MARKER, p.getLocation(), 1);
		}
	},
	TRAIL_1_LIFTED(){
		@Override public void display(Player p, long time){
			//if(time%10 == 0) ParticleEffect.FOOTSTEP.display(0, 0, 0, 0, 1, p.getLocation().add(0,.1,0), 20);
			if(time%15 == 0) p.getWorld().spawnParticle(Particle.BLOCK_MARKER, p.getLocation().add(0,.1,0), 1);
		}
	},
	TRAIL_2(){
		@Override public void display(Player p, long time){
...
//			ParticleEffect.CLOUD.display(.3F, .1F, .3F, 0, 10, toRight, 13);
	...
	RANDPARTICLE(){
		@Override public void display(Player p, long time){
			...
			if(particle != Particle.SPELL_MOB_AMBIENT)
	...
	BEACON(){
		@Override public void display(Player p, long time){
			...
			if(time%2 == 0) p.getWorld().spawnParticle(Particle.BLOCK_DUST, p.getLocation().add(0,40,0), 1000, .1, 15, .1, 1);
		}
	},
	IMAGINE_(){
		@Override public void display(Player p, long time){
