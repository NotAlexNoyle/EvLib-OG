...
import plugin.extras.TextUtils.StrAndPxLen;
...
public class TabText {
	final static int CHAT_HEIGHT = 100;
	final static int CHAT_WIDTH = 320, MONO_WIDTH = 80, MAX_PLAYER_NAME_WIDTH = 96;
	...
	public static Component parse(String str, boolean mono, boolean flexFill) {
	...
	public static Component parse(String str, boolean mono, boolean flexFill, int[] tabs) {
	...
	public static Component parse(String str, boolean mono, boolean flexFill, double[] tabs) {
		...
		for (int i = 0; i < tt.lines.length; ++i) {
			...
			if (fields.length > tt.tabs.length) {
				...
				for (int j = 0; j < tt.tabs.length; ++j)
					newTabs[j] = tt.tabs[j];
				tt.tabs = newTabs;
				fixI = i;
			}
			...
			if (missingTabs > 0)
				fields[fields.length - 1] += repeat(missingTabs, '`');
		...
		if (flexFill) {
			double sum = 0;
			for (double w : tt.tabs)
				sum += w;
			double leftover = ((mono ? MONO_WIDTH : CHAT_WIDTH) - sum) / tt.tabs.length;
			for (int i = 0; i < tt.tabs.length; ++i)
				tt.tabs[i] += leftover;
		}
		return tt.getPage(0, mono, "<black>");
	}
	 ...
	 * 
	 * @param charsList  a string with the chars to be added (careful with unicode
	 *                   or ANSI chars, do some tests before)
	...
	public static String getPxSpaces(double pxLenGoal, boolean monospace, String resumeColor) {
		if (monospace)
			return repeat((int) pxLenGoal, ' ');
		StringBuilder builder = new StringBuilder();
		double pxLen = 0;
		while (pxLen < pxLenGoal - 0.5) {
			...
			if (needShift == 0) {
				...
				if (lenLeft >= 5) {
					builder.append("<bold> </bold>").append(resumeColor);
					pxLen += 5;
				} else {
					builder.append(W1_HALF_C);
					pxLen += 1;
				}
			} else if (needShift == 2) {
				if (lenLeft >= 10) {
				...
				if (lenLeft >= 15) {
					...
					builder.append(W3_HALF_C);
					pxLen += 3;
				}
			} else if (needShift == 1.5) {
				builder.append("<bold>").append(W1_HALF_C).append("</bold>").append(resumeColor);
				pxLen += 1.5;
			} else if (needShift == 2.5) {
				builder.append("<bold>").append(W2_HALF_C).append("</bold>").append(resumeColor);
				pxLen += 2.5;
			} else if (needShift == 3.5) {
				builder.append("<bold>").append(W3_HALF_C).append("</bold>").append(resumeColor);
				pxLen += 3.5;
			} else if (needShift == 0.5) {
				builder.append("<bold>").append(W4_HALF_C).append("</bold>").append(resumeColor);
				pxLen += 4.5;
			}
		}
	 ...
	 *                 if variable width (for chat area)
	 * @return desired formatted, tabbed page as a Component
	 */
	public Component getPage(int page, boolean monospace, String hideTabs) {
		...
		// Convert the final string to a Component using MiniMessage
		return MiniMessage.miniMessage().deserialize(outputLines.toString());
	}
	...
	/**
	 * Sort lines by column values, string or numeric.
	 * 
	 * @param keys a list of column numbers criteria where 1 represents first column
	 *             and so on. Negative numbers mean descending order.
	 */
	public void sortByFields(int... keys) {
		...
		for (int i = 0; i < lines.length; ++i) {
			...
			for (int j = 0; j < keys.length; ++j) {
				...
				// Reverse field char values if reverse order
				if (desc2) {
					field = "";
