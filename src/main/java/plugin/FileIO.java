...
public final class FileIO {
	...
	// TODO: remove public? (only user: EvLib/extras/WebUtils.java)
	static public String DIR = EV_DIR;
		...
		if(srcDir.isDirectory()) {
			...
			for(final File file : srcDir.listFiles()) {
				...
				try{
					...
					Files.move(file.toPath(), new File(destDir.getPath()+"/"+file.getName()).toPath(), StandardCopyOption.COPY_ATTRIBUTES, StandardCopyOption.REPLACE_EXISTING);
	...
	public static void verifyDir(Plugin evPl) {
		...
		// merge with EvFolder
		else if(new File(CUSTOM_DIR).exists()) {
	...
	public static YamlConfiguration loadConfig(JavaPlugin pl, String configName, InputStream defaultConfig, boolean notifyIfNew) {
		...
		if(loadNewConfig) {
			...
			if(notifyIfNew) {
				...
				pl.getLogger().info("Could not locate " + configName + " file!");
	...
	public static String loadResource(Object pl, String filename, String defaultContent) {
		...
		try {
			...
			if(inputStream == null) {
				...
				inputStream = pl.getClass().getClassLoader().getResourceAsStream("/" + filename);
	...
	String[] getResourceListing(Class<?> clazz, String path) throws URISyntaxException, IOException {
		...
		if(dirURL.getProtocol().equals("jar")) {
			...
			try (JarFile jar = new JarFile(URLDecoder.decode(jarPath, "UTF-8"))) {
				...
				// Avoid duplicates in case it is a sub-directory.
